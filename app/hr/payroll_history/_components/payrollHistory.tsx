...
import { Payroll, PayrollService } from '@/app/services/hrServices/payrollService';
...
<Table className="min-w-full bg-white">
  <TableHeader>
    <TableRow className="border-t border-gray-200">
      ...
      <TableHead className="py-2 px-4 text-center">Month</TableHead>
      <TableHead className="py-2 px-4 text-center">Working Days</TableHead>
      <TableHead className="py-2 px-4 text-center">Paid Leaves</TableHead>
      <TableHead className="py-2 px-4 text-center">Sub Total</TableHead>
      <TableHead className="py-2 px-4 text-center">Tax</TableHead>
      <TableHead className="py-2 px-4 text-center">Net Salary</TableHead>
    </TableRow>
  </TableHeader>
  <TableBody>
    {payroll.length > 0 ? (
      payroll.map((entry, index) => (
        <TableRow key={entry.id} className={index % 2 === 0 ? 'border-t border-gray-200' : ''}>
          <TableCell className="py-2 px-4 text-center">{entry.year}</TableCell>
          <TableCell className="py-2 px-4 text-center">{entry.month}</TableCell>
          <TableCell className="py-2 px-4 text-center">{entry.working_days}</TableCell>
          <TableCell className="py-2 px-4 text-center">{entry.paid_leaves}</TableCell>
          <TableCell className="py-2 px-4 text-center">Rs.{entry.sub_total?.toFixed(2) || 'N/A'}</TableCell>
          <TableCell className="py-2 px-4 text-center">Rs.{entry.tax?.toFixed(2) || 'N/A'}</TableCell>
          <TableCell className="py-2 px-4 text-center">Rs.{entry.net_salary?.toFixed(2) || 'N/A'}</TableCell>
        </TableRow>
      ))
    ) : (
      <TableRow>
        <TableCell colSpan={10} className="py-2 px-4 text-center">No payroll data available</TableCell>
      </TableRow>
    )}
  </TableBody>
</Table>
