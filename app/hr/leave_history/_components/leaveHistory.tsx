              ...
              <Table className="min-w-full bg-white">
                <TableHeader>
                  <TableRow className="border-t border-gray-200">
                    <TableHead className="py-2 px-4 text-left">Start Date</TableHead>
                    <TableHead className="py-2 px-4 text-left">End Date</TableHead>
                    <TableHead className="py-2 px-4 text-left">Reason</TableHead>
                  ...
                  {leave.length > 0 ? (
                    leave.map((entry, index) => (
                      <TableRow key={entry.id} className={index % 2 === 0 ? 'border-t border-gray-200' : ''}>
                        <TableCell className="py-2 px-4">{entry.start_date}</TableCell>
                        <TableCell className="py-2 px-4">{entry.end_date}</TableCell>
                        <TableCell className="py-2 px-4">{entry.reason}</TableCell>
                        <TableCell className="py-2 px-4">{entry.type}</TableCell>
                        <TableCell className="py-2 px-4">{entry.status}</TableCell>
                      </TableRow>
                    ))
                  ) : (
                    <TableRow>
                      <TableCell colSpan={5} className="py-2 px-4 text-center">No leave data available</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
