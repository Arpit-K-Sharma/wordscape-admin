...
const LeftoverForm: React.FC<LeftoverFormProps> = ({ orderId, onSubmit }) => {
    ...
    const form = useForm({
        defaultValues: {
            order_id: orderId,
            items: [{ inventory_id: '', item_id: '', quantity: 0, reason: '', type: '' }]
        }
    });
            ...
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8">
                <ScrollArea className="h-[400px] rounded-md border ">
                    {fields.map((field, index) => (
                        <div key={field.id} className="flex flex-col space-y-4 shadow-md p-[10px] rounded-md mb-4">
                            <div className=''>
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            >
                                                ...
                                                <SelectContent>
                                                    {inventory
                                                        .find(inv => inv._id === form.getValues(`items.${index}.type`))
                                                        ?.item.map(item => (
                                                            <SelectItem key={`${form.getValues(`items.${index}.type`)}|${item._id}`} value={`${form.getValues(`items.${index}.type`)}|${item._id}`}>
                                                                {item.itemName}
                                                            </SelectItem>
                                                        ))
                                                    }
                ...
                <div className="flex space-x-4">
                    <div>
                        <Button type="button"
                            onClick={() => append({ item_id: '', inventory_id: '', quantity: 0, reason: '', type: '' })}>
