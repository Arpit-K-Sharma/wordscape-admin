...
import { ApprovedOrders, User, Cover, Paper, InventoryItem } from "../Schema/dashboardSchema"; 

const Dashboard = () => {
  const [approvedOrders, setApprovedOrders] = useState<ApprovedOrders[]>([]);
  const [user, setUser] = useState<User | null>(null);
  const [coverTreatment, setCoverTreatment] = useState<Cover | null>(null);
  ...
  useEffect(() => {
    const fetchApprovedOrders = async () => {
      try {
        ...
        const poCreatedOrders = orders.filter((order) => order.purchase_order_created === true);
        setPoCreated(poCreatedOrders.length);
        const poCreatedNotOrders = orders.filter((order) => order.purchase_order_created === null);
    ...
    const fetchRelatedData = async () => {
      if (approvedOrders.length > 0) {
        try {
          ...
          const coverTreatment = await dashboardService.fetch_cover_treatment(approvedOrders[0].coverTreatment);
          setCoverTreatment(coverTreatment);

          const innerPaper = await dashboardService.fetch_paper(approvedOrders[0].innerPaper);
          setInnerPaper(innerPaper);

          const outerPaper = await dashboardService.fetch_paper(approvedOrders[0].outerPaper);
              ...
              <HoverCardContent className="w-[300px] rounded-[20px]">
                <div className="p-[10px] items-center justify-center font-archivo">
                  ...
                  <p className=" ml-[20px] text-left text-gray-600 text-[15px]">
                    Welcome to the Inventory Dashboard! This page provides an overview of your inventory, and you can create purchase orders from here.
                  </p>
                </div>
              </HoverCardContent>
            </HoverCard>
          ...
          <CardContent>
            <AnimatePresence mode="wait">
              {inventoryItems[currentIndex] && (
                ...
                >
                  <div className="text-2xl font-bold">{inventoryItems[currentIndex].item[0].itemName}</div>
                  <p className="text-xs text-muted-foreground">
                    Type: {inventoryItems[currentIndex].type} | Availability: {inventoryItems[currentIndex].item[0].availability}
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <Table className="w-full text-sm text-left rtl:text-right text-white">
              <TableHeader className="text-xs text-black uppercase bg-gray-100 shadow-md">
                <TableRow>
                  ...
                  <TableHead className="px-6 py-3 text-[15px] text-black font-bold">Order Id</TableHead>
                  <TableHead className="px-6 py-3 text-[15px] text-black font-bold">Date</TableHead>
                  <TableHead className="px-6 py-3 text-[15px] text-black font-bold">Customer Name</TableHead>
                  <TableHead className="px-6 py-3 text-[15px] text-black font-bold">Estd. Cost</TableHead>
                  <TableHead className="px-6 py-3 text-[15px] text-black font-bold">View Details</TableHead>
                  <TableHead className="px-6 py-3 text-[15px] text-black font-bold">Action</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {poStatus === "created" ? (
                            ...
                            <SheetContent className="w-[800px]">
                              <SheetHeader>
                                ...
                                <SheetDescription className="font-bold text-[15px] shadow-sm border-black p-[10px] rounded-md">
                                  <Table>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold text-gray-600">Order Id</TableHead>
                                      <TableHead>{order._id}</TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold  text-gray-600">Customer Name</TableHead>
                                      <TableHead><p>{user?.fullName}</p></TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold  text-gray-600">Date</TableHead>
                                      <TableHead>{order.date}</TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold  text-gray-600">Paper Size</TableHead>
                                      <TableHead>{order.paperSize}</TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold  text-gray-600">Pages</TableHead>
                                      <TableHead>{order.pages}</TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold  text-gray-600">Quantity</TableHead>
                                      <TableHead>{order.quantity}</TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold  text-gray-600">Binding</TableHead>
                                      <TableHead>{order.binding}</TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold  text-gray-600">Cover Treatment</TableHead>
                                      <TableHead>{coverTreatment?.coverTreatmentType}</TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold  text-gray-600">Inner Paper</TableHead>
                                      <TableHead>{innerPaper?.paperType}</TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold  text-gray-600">Outer Paper</TableHead>
                                      <TableHead>{outerPaper?.paperType}</TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold  text-gray-600">Ink Type</TableHead>
                                      <TableHead>{order.inkType}</TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold  text-gray-600">Delivery Option</TableHead>
                                      <TableHead>{order.deliveryOption}</TableHead>
                                    </TableRow>
                                    <TableRow>
                                      <TableHead className="p-[10px] font-semibold text-gray-600">Status</TableHead>
