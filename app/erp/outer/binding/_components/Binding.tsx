...
import {
  ...
  addBinding,
  updateBinding,
} from "../../../../services/erpServices/bindingService";
...
export default function BindingPage() {
  ...
  const handleSave = (row: Binding) => {
    const updatedData = {
      bindingType: (
        document.getElementById(`binding_type_${row.id}`) as HTMLInputElement
      ).value,
      rate: parseFloat(
        (document.getElementById(`rate_${row.id}`) as HTMLInputElement).value
      ),
    };
          ...
          <div className="flex items-center justify-center mb-6">
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>
                    Manage binding types and rates for your printing projects.
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-[#131527] hover:bg-[#131527]">
                  <TableHead className="w-[15%] text-white text-center">
                    S.N
                  </TableHead>
                  <TableHead className="w-[35%] text-white text-center">
                    Binding Type
                  </TableHead>
                  <TableHead className="w-[25%] text-white text-center">
                    Rate
                  </TableHead>
                  <TableHead className="w-[25%] text-white text-center">
                    Actions
              ...
              <TableBody>
                {bindingDataState &&
                  bindingDataState.length > 0 &&
                  bindingDataState.map((row) => (
                    <TableRow key={row.id} className="hover:bg-gray-50">
                      <TableCell className="font-medium text-center">
                        <span
                          className="truncate block w-[120px]"
