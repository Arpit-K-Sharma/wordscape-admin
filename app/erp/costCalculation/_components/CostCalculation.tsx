...
import { getPaperSizes, getSheetSizes, getPapers, getPlates, getBindings, getLaminations, getCoverTreatments, getInks } from '../../../services/erpServices/costCalculationService';
...
export default function CostCalculationPage() {
  ...
  const [selectedOuterLaminationType, setSelectedOuterLaminationType] = useState('');
  ...
  useEffect(() => {
    ...
    if (selectedLaminationType) getRateForLaminationType(selectedLaminationType);
    if (selectedCoverTreatmentType) getRateForCoverTreatment(selectedCoverTreatmentType);
    ...
    if (selectedOuterLaminationType) getRateForOuterLaminationType(selectedOuterLaminationType);
    if (plateSize) getRatePlate(plateSize);
  }, [selectedBindingType, selectedLaminationType, selectedCoverTreatmentType, selectedPaperType, outerSelectedPaperType, selectedOuterLaminationType, plateSize]);

  const handlePaperSizeChange = (value: string) => {
    setPaperSize(value);
    const selectedPaperSize = paperSizes.find(size => size.paperSize === value);
  ...
  const getRateForPaper = async (selectedPaperType: string) => {
    try {
      ...
      const selectedPaper = papers.find(p => p.paperType === selectedPaperType);
      if (selectedPaper) {
  ...
  const getRateForOuterPaper = async (outerSelectedPaperType: string) => {
    if (outerSelectedPaperType) {
      try {
        const papers = await getPapers();
        const outPaper = papers.find(p => p.paperType === outerSelectedPaperType);
        if (outPaper) {
  ...
  const getRateForBindingType = async (selectedBindingType: string) => {
    try {
      ...
      const selectedBinding = bindings.find(b => b.bindingType === selectedBindingType);
      if (selectedBinding) {
  ...
  const getRateForLaminationType = async (selectedLaminationType: string) => {
    try {
      ...
      const selectedLamination = laminations.find(l => l.laminationType === selectedLaminationType);
  ...
  const getRateForCoverTreatment = async (selectedCoverTreatmentType: string) => {
    try {
      const coverTreatments = await getCoverTreatments();
      const selectedCoverTreatment = coverTreatments.find(ct => ct.coverTreatmentType === selectedCoverTreatmentType);
  ...
  const getRateForOuterLaminationType = async (selectedOuterLaminationType: string) => {
    try {
      const laminations = await getLaminations();
      const selectedOuterLamination = laminations.find(l => l.laminationType === selectedOuterLaminationType);
  ...
  const generateThicknessOptions = (selectedPaperType: string): number[] => {
    const selectedPaper = paperTypes.find(paper => paper.paperType === selectedPaperType);
    if (!selectedPaper) return [];

    const options: number[] = [];
    for (let thickness = selectedPaper.minThickness; thickness <= selectedPaper.maxThickness; thickness += 10) {
      options.push(thickness);
    }
    return options;
  };

  const fitPapers = (sheetBreadth: number, sheetLength: number, paperBreadth: number, paperLength: number): number => {
    ...
    const fitNormal = calculateFit(sheetBreadth, sheetLength, paperBreadth, paperLength);
    const fitRotated = calculateFit(sheetBreadth, sheetLength, paperLength, paperBreadth);
    console.log("this is the dataaaa", fitNormal, fitRotated)
    return Math.max(fitNormal, fitRotated) * 2;
  };

  const fitPlate = (plateBreadth: number, plateLength: number, paperBreadth: number, paperLength: number): number => {
    console.log("this is the data", plateBreadth, plateLength, paperBreadth, paperLength)
    const calculatePlate = (pb: number, pl: number, pab: number, pal: number) => {
      const fitHorizontally = Math.floor(pb / pab);
      const fitVertically = Math.floor(pl / pal);
      return fitHorizontally * fitVertically;
    };

    const fitNormal = calculatePlate(plateBreadth, plateLength, paperBreadth, paperLength);
    const fitRotated = calculatePlate(plateBreadth, plateLength, paperLength, paperBreadth);
    return Math.max(fitNormal, fitRotated);
  };

  const totalPlate = (pages: number, plateFit: number, selectedInkType: string): number => {
    const totalNo = Math.ceil(pages / plateFit);
    return selectedInkType === "CMYK" ? totalNo * 4 : totalNo;
  };

  const reamCalc = (selectedPaperThickness: number, sheetValue: number): number => {
  ...
  const innerPaperCost = (
    ...
    let calc = ((sheetValue * selectedPaperThickness * paperPrice) / 3100) * totalReams(pages, quantity, paperFit);
  ...
  const totalReams = (pages: number, quantity: number, paperFit: number): number => {
    return Math.ceil(0.05 * Math.floor((pages * quantity) / paperFit / 500) + Math.floor((pages * quantity) / paperFit / 500));
  };
  ...
  const laminationFinal = (laminationCost: number, sheetLength: number, sheetBreadth: number, quantity: number): number => {
    return laminationCost * (sheetLength / 2) * (sheetBreadth / 2) * quantity;
  };
  const totalSheets = (quantity: number, pages: number, paperFit: number): number => {
    return Math.round(quantity * pages) / paperFit + (0.05 * (Math.round(quantity * pages))) / paperFit;
  ...
  const calculateLamination = (laminationPrice: number, sheetLength: number, sheetBreadth: number, quantity: number, length: number, breadth: number): number => {
    return laminationPrice * (sheetLength || length) * (sheetBreadth || breadth) * quantity;
  }
      ...
      <div className="cost-calc-container bg-gray-100 w-[60%] min-h-screen border flex">
                  ...
                  <div className="cost-box-m">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <Select onValueChange={handlePaperThicknessChange} value={selectedPaperThickness}>
                        ...
                        <SelectContent>
                          {generateThicknessOptions(selectedPaperType).map((thickness) => (
                      ...
                      <Select onValueChange={handlePaperOuterThicknessChange} value={selectedOuterPaperThickness}>
                        ...
                        <SelectContent>
                          {generateThicknessOptions(outerSelectedPaperType).map((thickness) => (
                            <SelectItem key={thickness} value={thickness.toString()}>
                              {thickness}
                            </SelectItem>
                          ))}
                  ...
                  <div className="cost-box-2">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      ...
                      <Select onValueChange={handleSheetSizeChange} value={sheetSize}>
                        ...
                        <SelectContent>
                          {sheetSizes.map((size) => (
                            <SelectItem key={size.sheetSizeId} value={size.sheetSize}>
                              {size.sheetSize}
                            </SelectItem>
                          ))}
                        </SelectContent>
        ...
        <div className="drawer-test fixed right-0 top-0 h-full w-1/4 shadow-lg overflow-y-auto transition-transform transform translate-x-0">
          <DrawerTest
            ...
            totalReams={totalReams(parseInt(pages), parseInt(quantity), paperFit)}
            ...
            totalCost={totalCost}
            noPlate={noPlate}
            requiredSheet={totalSheets(parseInt(quantity), parseInt(pages), paperFit)}
            paperFit={paperFit}
            ...
            totalSheets={totalSheets(parseInt(quantity), parseInt(pages), paperFit)}
            costReam={reamCalc(parseInt(selectedPaperThickness), parseFloat(sheetValue))}
